...
from .tools import img_transform, normalize_img, gen_dx_bx, denormalize_img


class LunarData(torch.utils.data.Dataset):
    def __init__(self, dataroot, is_train, data_aug_conf, grid_conf, train_test_val, use_tts=False):
        ...
        if not use_tts:
            ...
            self.cam_img_paths = self.get_imgs_paths_by_cam_tts()
            self.binimg_paths = self.get_gt_imgs_paths_tts()
    ...
    def get_imgs_paths_by_cam_tts(self):
        img_paths = {}
        for cam in self.cams:
                paths = sorted(glob(self.dataroot + '*/solo*/sequence.0/step*.' + cam + '.png'))
                img_paths[cam] = paths
                if len(paths) == 0:
                    raise FileNotFoundError(f"images not found for {cam}")
        return img_paths
    

    def get_gt_imgs_paths_tts(self):
        paths = sorted(glob(self.dataroot + '*/solo*/sequence.0/step*.semantic segmentation.png'))
        if len(paths) == 0:
            raise FileNotFoundError("binary images not found")
        return paths
    ...
    def select_indexes(self, train_test_val):
        ...
        if not self.use_tts:
            ...
            train_indexes, val_indexes = train_test_split(all_indexes, test_size=0.1, random_state=42)
            ...
            if train_test_val == 'train':
                ...
                raise ValueError("train_test_val must be 'train'or 'val'")
        

    def load_cam_intrinsics(self):
        try:
            with open('LunarSim/cam_intrinsics_nusc.json', 'r') as f:
...
def compile_data_tts(dataroot, data_aug_conf, grid_conf, bsz, nworkers):

    traindata = SegmentationData(dataroot, is_train=True, data_aug_conf=data_aug_conf,
                         grid_conf=grid_conf, train_test_val='train', use_tts=True)

    valdata = SegmentationData(dataroot, is_train=False, data_aug_conf=data_aug_conf,
                       grid_conf=grid_conf, train_test_val='val', use_tts=True)
